...
// autodiff includes
#include <autodiff/forward/forward-v3.hpp>
// #include <autodiff/forward/eigen.hpp>
using namespace autodiff;
using namespace autodiff::forward3;

template<typename T>
auto approx(T&& expr) -> Approx
{
    const double zero = std::numeric_limits<double>::epsilon();
    return Approx(eval(std::forward<T>(expr))).margin(zero);
}

namespace autodiff::forward {

template<typename U, enableif<isExpr<U>>...>
auto operator==(U&& l, const Approx& r) { return eval(std::forward<U>(l)) == r; }
...
TEST_CASE("autodiff::dual tests v3", "[dual]")
{
    dual x = 100;
    dual y = 10;
