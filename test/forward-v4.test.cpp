...
// autodiff includes
#include <autodiff/forward/forward-v4.hpp>
...
TEST_CASE("dual tests", "[dual]")
{
    ...
    CHECK( y[0] == Approx( x[0] ) );
    CHECK( y[1] == Approx( x[1] ) );
    CHECK( y[2] == Approx( x[2] ) );
    CHECK( y[3] == Approx( x[3] ) );
    CHECK( y[4] == Approx( x[4] ) );
    ...
    CHECK( z[0] == Approx( x[0] + y[0] ) );
    CHECK( z[1] == Approx( x[1] + y[1] ) );
    CHECK( z[2] == Approx( x[2] + y[2] ) );
    CHECK( z[3] == Approx( x[3] + y[3] ) );
    CHECK( z[4] == Approx( x[4] + y[4] ) );

    z = x + 1.0;

    CHECK( z[0] == Approx( x[0] + 1.0 ) );
    CHECK( z[1] == Approx( x[1] ) );
    CHECK( z[2] == Approx( x[2] ) );
    CHECK( z[3] == Approx( x[3] ) );
    CHECK( z[4] == Approx( x[4] ) );
    ...
    CHECK( z[0] == Approx( x[0]*y[0] ) );
    CHECK( z[1] == Approx( x[1]*y[0] + x[0]*y[1] ) );
    CHECK( z[2] == Approx( x[2]*y[0] + 2*x[1]*y[1] + x[0]*y[2] ) );
    CHECK( z[3] == Approx( x[3]*y[0] + 3*x[2]*y[1] + 3*x[1]*y[2] + x[0]*y[3] ) );
    CHECK( z[4] == Approx( x[4]*y[0] + 4*x[3]*y[1] + 6*x[2]*y[2] + 4*x[1]*y[3] + x[0]*y[4] ) );

    z = x / y;

    CHECK( z[0] == Approx( (x[0])/y[0] ) );
    CHECK( z[1] == Approx( (x[1] - y[1]*z[0])/y[0] ) );
    CHECK( z[2] == Approx( (x[2] - y[2]*z[0] - 2*y[1]*z[1])/y[0] ) );
    CHECK( z[3] == Approx( (x[3] - y[3]*z[0] - 3*y[2]*z[1] - 3*y[1]*z[2])/y[0] ) );
    CHECK( z[4] == Approx( (x[4] - y[4]*z[0] - 4*y[3]*z[1] - 6*y[2]*z[2] - 4*y[1]*z[3])/y[0] ) );

    z = 3.0 / y;

    CHECK( z[0] == Approx( 3.0/y[0] ) );
    CHECK( z[1] == Approx( -(y[1]*z[0])/y[0] ) );
    ...
    CHECK( y[0] == Approx( sin(x[0]) ) );
    CHECK( z[0] == Approx( cos(x[0]) ) );
    CHECK( y[1] == Approx(  x[1] * z[0] ) );
    CHECK( z[1] == Approx( -x[1] * y[0] ) );
    CHECK( y[2] == Approx(  x[2] * z[0] + x[1] * z[1] ) );
    CHECK( z[2] == Approx( -x[2] * y[0] - x[1] * y[1] ) );
    CHECK( y[3] == Approx(  x[3] * z[0] + 2*x[2] * z[1] + x[1] * z[2] ) );
    CHECK( z[3] == Approx( -x[3] * y[0] - 2*x[2] * y[1] - x[1] * y[2] ) );
    CHECK( y[4] == Approx(  x[4] * z[0] + 3*x[3] * z[1] + 3*x[2] * z[2] + x[1] * z[3] ) );
    CHECK( z[4] == Approx( -x[4] * y[0] - 3*x[3] * y[1] - 3*x[2] * y[2] - x[1] * y[3] ) );

    y = tan(x);
    z = 1/(cos(x)*cos(x)); // sec(x)**2

    CHECK( y[0] == Approx( tan(x[0]) ) );
    CHECK( z[0] == Approx( 1/(cos(x[0])*cos(x[0])) ) );
    CHECK( y[1] == Approx( x[1]*z[0] ) );
    CHECK( z[1] == Approx( 2*(y[0]*y[1]) ) );
    CHECK( y[2] == Approx( x[2]*z[0] + x[1]*z[1] ) );
    CHECK( z[2] == Approx( 2*(y[0]*y[2] + y[1]*y[1]) ) );
    CHECK( y[3] == Approx( x[3]*z[0] + 2*x[2]*z[1] + x[1]*z[2] ) );
    CHECK( z[3] == Approx( 2*(y[0]*y[3] + 2*y[1]*y[2] + y[2]*y[1]) ) );
    CHECK( y[4] == Approx( x[4]*z[0] + 3*x[3]*z[1] + 3*x[2]*z[2] + x[1]*z[3] ) );
    CHECK( z[4] == Approx( 2*(y[0]*y[4] + 3*y[1]*y[3] + 3*y[2]*y[2] + y[1]*y[3]) ) );
